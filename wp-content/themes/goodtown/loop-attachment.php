...
<?php if ( have_posts() ) while ( have_posts() ) : the_post(); ?>
				...
				<?php if ( ! empty( $post->post_parent ) ) : ?>
					<p class="page-title"><a href="<?php echo esc_url( get_permalink( $post->post_parent ) ); ?>" title="<?php echo esc_attr( sprintf( __( 'Return to %s', 'goodtown' ), strip_tags( get_the_title( $post->post_parent ) ) ) ); ?>" rel="gallery"><?php
						/* translators: %s - title of parent post */
						printf( __( '<span class="meta-nav">&larr;</span> %s', 'goodtown' ), get_the_title( $post->post_parent ) );
					?></a></p>
				<?php endif; ?>
				...
				<div id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
					...
					<div class="entry-meta">
						<?php
							printf( __( '<span class="%1$s">By</span> %2$s', 'goodtown' ),
								'meta-prep meta-prep-author',
								sprintf( '<span class="author vcard"><a class="url fn n" href="%1$s" title="%2$s" rel="author">%3$s</a></span>',
									get_author_posts_url( get_the_author_meta( 'ID' ) ),
									esc_attr( sprintf( __( 'View all posts by %s', 'goodtown' ), get_the_author() ) ),
									get_the_author()
								)
							);
						?>
						...
						<?php
							printf( __( '<span class="%1$s">Published</span> %2$s', 'goodtown' ),
							...
							if ( wp_attachment_is_image() ) {
								...
								printf( __( 'Full size is %s pixels', 'goodtown' ),
									sprintf( '<a href="%1$s" title="%2$s">%3$s &times; %4$s</a>',
										esc_url( wp_get_attachment_url() ),
										esc_attr( __( 'Link to full-size image', 'goodtown' ) ),
										$metadata['width'],
										$metadata['height']
									)
								);
...
?>
						<p class="attachment"><a href="<?php echo esc_url( $next_attachment_url ); ?>" title="<?php the_title_attribute(); ?>" rel="attachment"><?php
							 ...
							 */
							$attachment_width  = apply_filters( 'goodtown_attachment_size', 900 );
							/**
							 ...
							 */
							$attachment_height = apply_filters( 'goodtown_attachment_height', 900 );
...
<?php wp_link_pages( array( 'before' => '<div class="page-link">' . __( 'Pages:', 'goodtown' ), 'after' => '</div>' ) ); ?>
					...
					<div class="entry-utility">
						<?php goodtown_posted_in(); ?>
						<?php edit_post_link( __( 'Edit', 'goodtown' ), ' <span class="edit-link">', '</span>' ); ?>
					</div><!-- .entry-utility -->
				</div><!-- #post-## -->
